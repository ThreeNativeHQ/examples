# AGENTS.md — pirate-authoring-retest
...
1. **Before planning, call it once with the user's complete game request.** A broad request such
   as *"make a pirate game"* returns the compact cross-system set the installed engine associates
   with that genre.
2. **List each mechanic you intend to build, then call it once per mechanic** — ocean buoyancy,
   cloth sails, cannon smoke, crew pathfinding, weapon attachment, audio, and terrain are separate
   situations. Use the returned `matchedSituation` to confirm why each result was selected.
