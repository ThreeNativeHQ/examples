...
export function setupLighting(
  ...
  scene.add(new HemisphereLight(palette.skyHigh, palette.shadow, 2.4));

  const key = new DirectionalLight(palette.accent, 4.2);
  ...
  const rim = new DirectionalLight(palette.player, 1.5);
  rim.position.set(-5, 3, -6);
  scene.add(rim);

  scene.add(new AmbientLight(palette.skyHigh, 0.9));
  return { updateSun };
}
