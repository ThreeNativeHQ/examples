...
const game = defineGame<GameState, IPhysicsContext>({
  input: {
    ...
    // One camera-intent axis, read as `input.axis("zoom")`. The wheel on a desktop and a
    // two-finger pinch on a phone arrive as the same number, so the rig has no platform branch.
    zoom: { scroll: true, pinch: true, keys: [] },
  },
