# AGENTS.md — threenative-hq

**This game is a window onto the machine it runs on.** Every desk is a live Claude Code or Codex
session; the worker's pose is that session's state. Read `brief.md`, then the PRD it was built
against: `docs/PRDs/agent-leverage/PRD-313-*.md` in the engine repository.

## What lives where

| Thing | Where |
| --- | --- |
| The look — room, furniture, palette, lighting, framing | `src/render/` |
| Session state -> pose, as a pure function | `src/office/states.ts` |
| The worker entity (rig, mixer, tint) | `src/office/Worker.ts` |
| Scene wiring | `src/scenes/Office.ts` |
| Asset trimming (re-runnable) | `tools/trim-clips.mjs` |

## Rules this game holds itself to

1. **Never fake a session.** A worker on screen means a session the bridge actually observed. A
   missing clip, a dead bridge or an unknown state fails loudly — a mannequin frozen in its bind
   pose looks exactly like a worker that is calmly idle, and that is the one lie this game cannot
   tell.
2. **The clip mapping is the whole state->look decision** and lives in one pure file, so a test
   can assert every row without a renderer.
3. **Assets come from the asset MCP**, never from a hand-copied download: the mannequin and its
   clips are CC0 Quaternius, fetched with `asset_download_bundle_entry` and trimmed by
   `tools/trim-clips.mjs`.
4. Engine-shaped friction goes in `FRICTION.md` with the evidence path, not into a local
   workaround that the next game has to write again.
