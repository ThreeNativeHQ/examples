// Bayview's surfaces: white-washed plaster, blue joinery, stone lanes, quay masonry and timber.
//
// PRD-227's device profile resolved the remaining steady-state shader-graph work to this game's
// triplanar TSL materials. Phase 4 keeps the authored colour/material families and all geometry,
// but static town surfaces are bounded-cost unlit materials. Imported characters, weapons, water
// and small metal props remain lit standard materials, so the scene keeps depth and specular cues.
import {
  DoubleSide,
  FrontSide,
  MeshBasicMaterial,
...
/**
 * A bounded-cost town surface.
 *
 * PRD-227's symbolized Pixel profile resolved the remaining steady-state IC population to the
 * TSL node-material shader graph. Bayview therefore keeps its authored palette and sidedness, but
 * no longer asks Three.js to walk two triplanar samples plus procedural noise for every material
 * on every frame. `maps` and `tileMetres` stay in the signature so asset loading and the town
 * builder remain one portable source while this game-owned Phase 4 fallback is active.
 */
