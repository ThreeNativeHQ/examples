...
# AGENTS.md — capability-pirate-proof-v2
...
1. **Before planning, infer the concrete gameplay mechanics implied by the request.** A genre or
   ...
   combat, camera, audio, and UI mechanics that game actually needs. Preserve the request's
   distinctive fantasy: choose the smallest loop that uses its characteristic setting, traversal
   medium, or simulation instead of making a generic character game with themed props. Search
   those implied environment and traversal mechanics even when the user did not name their engine
   terms. If two genuinely different core loops remain equally plausible, ask one short question
   instead of assuming a preset.
2. **Call it once with the complete, mechanically explicit request using `scope: "request"`, then
   once per mechanic using `scope: "mechanic"`.** Use the returned `matchedSituation` to confirm
