...
for (const s of f.ships) console.log([s.id, s.src, s.length, s.beam, s.height, s.draught, s.flip, s.budget].join("\t"));
' | while IFS=$'\t' read -r id src length beam height draught flip budget; do
