...
const SCREENS: Record<ScreenName, string> = { loading: "loading", briefing: "briefing", flight: "flight-ui", debrief: "debrief" };
...
class DomShell implements IShell {
  ...
  constructor() {
    ...
    const buttons: Record<string, () => void> = {
      ...
      "btn-command": () => emit({ kind: "overlay", id: "command-overlay" }),
      "close-pause": () => emit({ kind: "close" }),
      "close-map": () => emit({ kind: "close" }),
      "close-command": () => emit({ kind: "close" }),
      resume: () => emit({ kind: "close" }),
      "btn-camera": key("KeyC"),
      "btn-report": key("KeyR"),
      "btn-bomb": key("KeyB"),
      "btn-next-target": () => emit({ kind: "next-target" }),
    ...
    this.on($("command-buttons"), "click", picked("command", (id) => ({ kind: "command", id })));
    this.on($("contact-list"), "click", picked("contact", (id) => ({ kind: "target", id })));
    this.on($("big-map"), "click", (event) => {
      ...
      const nearest = (this.view?.mapItems ?? []).map((p) => ({ ...p, d: Math.hypot(p.x - x, p.y - y) })).sort((a, b) => a.d - b.d)[0];
  ...
  loadout(view: ILoadoutView): void {
    ...
    $("deck-loadout-note").textContent = view.deckNote;
  }
