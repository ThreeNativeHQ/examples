...
Evidence commands, run from `midway-open-pacific/`: **E1** `node scripts/check-flight.mjs`;
**E2** `bash tools/capture-lock.sh node tools/capture-lifecycle.mjs`; **E3**
`bash tools/capture-lock.sh node tools/capture-sortie.mjs` and
`MIDWAY_WIDTH=1400 MIDWAY_HEIGHT=800 MIDWAY_BURN=1 bash tools/capture-lock.sh node tools/capture-performance.mjs`.
E2 and E3 were run against an isolated copy of this tree served on a private port, because another
lane edits this checkout while gates run and its hot reloads break them mid-run.

- [x] AC-1 [local; actor: agent]: B releases stores through the scene and actual flight step sees bomb payload 544→90→45→0 kg or torpedo 1000→0 kg with matching drag; an empty/rejected release changes nothing and service restores the selected loadout. — Evidence: E1 `bombSequence [544,90,45,0]`, drag 0.003→0, torpedo 1000→0, rejected release leaves 0, service restores 544. E2 releases the torpedo through the real scene.
- [x] AC-2 [local; actor: agent]: Actual projectile hits and subsequent fire/sinking award player, ordered wing and unrelated ally contributions correctly, exactly once; near misses are labeled accurately and friendly damage earns no positive credit. — Evidence: E1 — allied-AI bomb scores 0 while still damaging the ship; an ordered wing hit scores `wingShipHits 1` and no personal hit; the player's own bomb scores +250/1; a splash counts `nearMisses 1` and no ship hit; a bomb on USS Hornet scores 0 and counts a friendly hit; a fire death credits the preserved last attacker once and invents no weapon hit.
- [x] AC-3 [local; actor: agent]: At and around the launch-disable boundary, actual launch eligibility, current observed target status and operation objective agree; disabled afloat ships are not reported sunk. Recovery uses its separately named eligibility. — Evidence: E1 — `deck 0.35` is operational and can launch, `0.3499` is neither and is not sunk. `LAUNCH_DECK`, `RECOVERY_DECK` and `DECK_FAILED` are the three named thresholds.
- [x] AC-4 [local; actor: agent]: Briefing selects strike, recon or Open Pacific; both start buttons honor it. Recon completes its objective only for a fresh transmitted carrier report, while a stale/escort-only report does not. — Evidence: E1 — an escort report and a stale sighting both leave the objective pending, a fresh carrier report completes it; the assignment is a briefing choice only. E2 selects `recon` from the real `#assignment-select` and reaches `objective: achieved`.
- [x] AC-5 [local; actor: agent]: A designated carrier's confirmed player/ordered-wing bomb or armed-torpedo direct hit advances the strike objective; unrelated allied fire does not. Recall/retask after release preserves recorded eligibility, and an unobserved hit waits for confirmation. A prematurely destroyed/unavailable target produces a valid retask or explicit return outcome. — Evidence: E1 — six cases: observed player hit completes; unstamped allied bomb does not; recall to RTB after release still credits; an ordered wing hit completes as a wing contribution; a stale contact leaves the hit `pending` until a fresh report confirms it; a target sunk by others releases the designation, and with no carrier left the objective becomes `unavailable`.
- [x] AC-6 [local; actor: agent]: Mission panel and map orders agree with actual phase and assignment; deck tip/manual identify Down as nose-up; rejected actions explain their requirement without hiding existing aiming/stability assistance. — Evidence: E3 frames `01-briefing.png`, `02-burning-carrier.png` and `03-approach.png` — the mission panel reads `04 / RECOVER — Bring your crew home — Objective complete …`; the deck tip now reads `gently hold ↓ (Down) to raise the nose`; the README's mouse-steering claim is gone; the squadron overlay carries a live assignment/target/wing line.
- [x] AC-7 [local; actor: agent]: Actual 1/2/3/4 commands update wing intent and the UI reports resulting aircraft behavior or unavailability; command acknowledgment alone never reports an attack as executed. — Evidence: E2 — after pressing `2` the wing line reads `WING / STRIKE — 1 en route · 1 forming. No valid strike target designated.`, derived from live aircraft tactics and targets, not from the order.
- [x] AC-8 [local; actor: agent]: H guides to a moving carrier's astern setup point before final; a newly unavailable deck triggers diversion/wave-off, while no available deck leaves a truthful message and no invalid autopilot target. — Evidence: E1 — the return course aims at the setup point 1600 m astern in the carrier's frame, not at the deck; disabling the assigned deck diverts to another usable one; disabling every deck cancels the assist, drops the autopilot and reports `NO AVAILABLE DECK` once.
- [x] AC-9 [local; actor: agent]: Approach cues and L eligibility agree for aligned/misaligned, high/low, fast/slow and gear states; assisted final flies continuously and manual steering cancels it. — Evidence: E1 — eight states (too fast, high, low, gear up, either side of the centreline, ahead of the deck, misaligned) each report their own cue and are each refused by the same `finalReady` gate that reported them; an aligned approach reports `READY FOR L` and is accepted; a stick input cancels the assist. The assisted final now flies to an arrestment, which it did not do before.
- [x] AC-10 [local; actor: agent]: Return reserve uses finite observed burn/route data; a leak or longer diversion reduces margin, and unavailable estimates never display infinite range or guaranteed recovery. — Evidence: E1 — no estimate before a burn sample or from a stationary sample; a tripled observed burn and a 14 km diversion each reduce the margin without adding fuel; zero, negative, infinite and NaN burn rates all yield `estimate unavailable`.
- [x] AC-11 [local; actor: agent]: Safe recovery, incomplete safe return and aircraft loss create distinct short-sortie results, preserving pre-repair fuel/damage and ownership-specific counters; replay resets them. Open Pacific still rearms and requires the full operation objective. — Evidence: E1 — `recovered` / `incomplete` / `lost` are three distinct results, arrival fuel 41 % and airframe 61 % are captured before the repair reset (which never runs for a short sortie), and Open Pacific still enters `service` and stays `playing`. E2/E3 show the debrief on screen: `CARRIER STRIKE / AFTER ACTION REPORT — Objective achieved — recovered`.
- [x] AC-12 [local; actor: agent]: A real hit creates a localized persistent scar/fire following the moving ship; a near miss creates sea effects without a false deck scar. Repeated impacts stay within 8 records/3 fire origins and clear on restart. — Evidence: E1 caps impacts at 8 in the ship frame with their attribution. E3 — three deck hits 71.5 m apart along the hull produce three scars at distinct hull positions, all at y 20.35 on the deck; the fourth weapon, a near miss alongside, produces none; fires come from the three most recent non-near-miss impacts.
- [x] AC-13 [local; actor: agent]: Captured cockpit/chase views at 1400×800 show distinguishable direct hits, splashes and burning damage; flight sightlines remain readable on balanced and low quality, and mission/approach guidance works with sound muted and keyboard navigation. — Evidence: E3 frames viewed at 1400×800 — `02-burning-carrier.png` shows three separate fire columns down Akagi's deck rather than one at the hull centre; `03-approach.png` keeps the cockpit sightline clear with the approach line in the mission panel; E2 cycles all three quality settings without error. The assignment control is a native `<select>` with a visible focus ring.
- [x] AC-14 [local; actor: agent]: Comparable before/after warm combat captures identify adapter, quality, resolution and workload; GPU p95 remains within 10% of baseline and ≤16.7 ms on this NVIDIA/Turing reference, with no unbounded effect/mesh growth after three restarts. — Evidence: E3, NVIDIA / Turing, 1400×800, pixel ratio 1, balanced, burning-carrier workload (7 burning ships, 28 recorded impacts), course held. Baseline (same tree with only this PRD's two render changes reverted): GPU median 4.20 ms, p95 **13.49 ms**, worst 14.61 ms, 91 draw calls. After: median 4.61 ms, p95 **13.67 ms** (+1.3 %), worst 14.93 ms, 90 draw calls, 16 scars drawn. Both ≤16.7 ms. Scars are budgeted by distance and quality, which is what brought an earlier +8 % p95 down. E2 restarts three times with mesh and crew counts unchanged.
- [x] AC-15 [local; actor: agent]: Existing launch, aircraft, affected combat/recovery lifecycle and type/build checks pass against the final installed packages; added assertions verify gameplay state, not only absence of console errors. — Evidence: `pnpm typecheck`, `pnpm exec vite build`, `node scripts/check-aircraft.mjs`, the installed playtest runner on `playtests/launch.playtest.json` (`--browser-recipe webgpu --headed`) and E1 all pass; E2 passes end to end with `errors: []`. Every assertion added here reads simulation or DOM state.
- [ ] AC-16 [local; actor: agent]: Two normal-entry airborne runs—one short recon and one short carrier strike—reach their correct recovered debrief in ≤12 elapsed minutes each using permitted assistance, with no world-state/HP/weapon/landing injection after start. — **Not met, with a measured cause.** With the player not defending, the simulation sinks every U.S. flight deck at **328.3 s** of battle time — measured identically on a copy of this tree with this PRD's changes reverted, so it is pre-existing attrition and not a regression. A fully passive guided return from the airborne start takes about 320 s, so the run ends in defeat before it can recover. `tools/capture-sortie-runs.mjs` is committed and reproduces this. The equivalent sorties do complete deterministically once the aircraft is inside the pattern: E1 flies a recon to `recovered` with its objective in **180.8 s** and a strike return to `incomplete` in **173.0 s**, both well inside the twelve-minute ceiling. Closing this needs either a player who defends the task force or a look at that attrition rate, which is outside this slice.
...
| E4 | `bash tools/capture-lock.sh node tools/capture-sortie.mjs` — the briefing choice, a designated attack, the approach line and the short debrief, with the frames a person has to look at. `bash tools/capture-lock.sh node tools/capture-sortie-runs.mjs` — two keys-only sorties from the airborne start. | Impact placement a screenshot cannot assert, and attainability without injection. |
