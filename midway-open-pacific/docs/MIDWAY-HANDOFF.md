...
| Gate | Result |
|---|---|
| `pnpm typecheck`, `pnpm exec vite build` | pass |
| `check-flight`, `check-aircraft`, `check-audio`, `check-carrier-assets`, `check-fleet` | pass |
| `tools/check-repair.mjs` in the browser | pass, including the mouse-never-steers assertions section 11.5 said had never been run |
| `launch`, `flight` and `briefing` playtest scenarios | all three pass, no failing check |
| `tools/capture-fleet.mjs` | pass |
| `tools/capture-deck.mjs` | pass |
| `tools/capture-lifecycle.mjs` | pass — the whole loop, see below |
| `tools/capture-performance.mjs` | pass, GPU median 3.57ms |

The lifecycle run covers: torpedo loadout flown off the deck and released; restart to the bomb
loadout; 15 aircraft up across both navies; a contact reported; **a player dive-bombing attack
that took Akagi from 340hp to 182.5 with a fire burning and her deck down to 0.3**; that carrier
sunk; a recovery approach; the loss state; all three graphics settings; and a restart that returns
to the briefing with the twelve-man deck party intact. No console or GPU error at any point.
States that had to be forced are named in the run's own output.

### Still open

- **Subjective audio listening (section 9)** — still not done. An agent cannot hear it; this needs João.
- **Native `--target` playtest** — cannot run. `@threenative/runtime-native` is not installed in this game and there is no `native-playtests/` directory, so `pnpm build:desktop` fails with "Cannot build a native target". Closing it means installing the native runtime tarball and authoring the two scenarios `package.json` already names.
- **Enterprise wears Hornet's "8".** Located precisely: image 59 of `hornet.glb`, a 1024² atlas used by material 59 (`Enterprise_cm056_ships_bell_124_32.002`) across 63,878 triangles of flight deck, with the two glyphs near (900, 660) and (96, 894). The blocky glyph is two stacked octagons, so a 6 or a 5 can be cut from it by erasing strokes rather than redrawing — but it needs a per-carrier material and texture clone, and a hand-drawn digit risks looking worse than the crisp one there now. Left deliberately, costed.
- The IJN carriers' island sides are Akagi's for all four at close range.
- The deck park is aft of the launch spot, which is correct, but it means the briefing view no longer has aircraft ahead of the player. If João wants the deck dressed there, the honest way is more aircraft aft, not aircraft overhanging the edge.
